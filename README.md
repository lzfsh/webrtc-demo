...
> 浏览器可以在 ws 握手阶段时，通过设置 `Set-WebSocket-Protocol` headers 头发送鉴权信息。服务端接收后，检查 headers 是否包含子协议鉴权信息，如果包含则进行鉴权认证，否则拒绝连接。

**一般情况下，不推荐这个方式发送敏感信息，比如鉴权信息。**在实际应用中，子协议包括以下几个主要用途：
