...
curl -s -X POST http://localhost:3000/api/user/list -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"username": "zhangsan"}' | jq '.'
curl -s -X POST http://localhost:3000/api/user/list -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"id": 1}' | jq '.'
